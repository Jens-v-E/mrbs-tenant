...
// Multi-Tenant: Benutzer muss eingeloggt und tenant_id muss vorhanden sein
if (
    !isset($_SESSION['user']) ||
    empty($_SESSION['user']) ||
...
// --- Multi-Tenant: Raumzugriff prüfen ---
// User-Tenant aus der Session
$user_tenant = (int)($_SESSION['tenant_id'] ?? 0);
$is_global_admin = !empty($_SESSION['is_global_admin']);

// Räume aus Formular (kann leer oder 1..n Einträge haben)
$rooms_to_check = $selected_rooms;

// Falls Einzelbuchung per URL (room-Parameter)
if (empty($rooms_to_check)) {
    $single_room = get_form_var('room', 'int');
    if (!empty($single_room)) {
        $rooms_to_check = [$single_room];
    }
}

// Tenant-Check für jeden Raum
if (!$is_global_admin) {
    foreach ($rooms_to_check as $room_id) {

        // Tenant des Raums ermitteln
        $room_tenant = db()->query1(
            "SELECT tenant_id FROM mrbs_room WHERE id = ? LIMIT 1",
            [$room_id]
        );

        // Wenn Raum keinem Tenant zugeordnet → blockieren
        if ($room_tenant <= 0) {
            showAccessDenied($view, $view_all, $year, $month, $day, $area, $room_id);
            exit;
        }

        // ⛔ Wenn Tenant != UserTenant → Zugriff verbieten
        if ($room_tenant != $user_tenant) {
            showAccessDenied($view, $view_all, $year, $month, $day, $area, $room_id);
            exit;
        }
    }
