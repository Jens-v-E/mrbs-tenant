...
//require_once __DIR__ .'/include/tenant.inc.php';

// Nur für DB-basierte Authentifizierung (inkl. Multi-Tenant-Variante)
if (!in_array($auth['type'], ['db', 'dbTenant']))
{
...
// $row = kompletter Datensatz des Users (Array)
// VARIANTE B: nur Admins (global oder tenant) sehen die Liste
function can_view_user(array $row)
{
  ...
  // Globaler Admin darf alle sehen
  if (!empty($_SESSION['is_global_admin']))
  {
    return true;
  }

  // Tenant-Admin: nur Benutzer im eigenen Tenant
  if (!empty($_SESSION['is_tenant_admin']))
  {
    $tenant_id_session = $_SESSION['tenant_id'] ?? null;
    $tenant_id_user    = $row['tenant_id'] ?? null;

    if ($tenant_id_session === null || $tenant_id_user === null)
    {
      return false;
    }

    return ((int)$tenant_id_user === (int)$tenant_id_session);
  }

  // Normale User sehen in VARIANTE B keine Liste
  return false;
}
...
// $row = kompletter Datensatz des Users (Array)
function can_edit_user(array $row)
{
  $mrbs_user = session()->getCurrentUser();
  if (!isset($mrbs_user))
  {
  ...
  // 1) Global Admin darf alle Benutzer bearbeiten
  if (!empty($_SESSION['is_global_admin']))
  {
    return true;
  }

  // 2) Tenant-Admin: nur Benutzer seines Tenants bearbeiten,
  //    und NICHT globale Admins anfassen
  if (!empty($_SESSION['is_tenant_admin']))
  {
    $tenant_id_session = $_SESSION['tenant_id'] ?? null;
    $tenant_id_user    = $row['tenant_id'] ?? null;
    ...
    // Tenant-Admin darf globale Admins nicht editieren
    if (!empty($row['is_global_admin']))
    {
      return false;
    }
  ...
  // 3) Standard-Fall: normale MRBS-Admins oder User selbst
  return (
    is_user_admin() ||
    (strcasecmp_locale($mrbs_user->username, $row['name']) === 0)
  );
}
...
function output_row($row)
{
  ...
  {
    $key = $field['name'];
    // NEW: Tenant-Admins sehen is_global_admin nicht in den Zeilen
    if ($key === 'is_global_admin' && !($_SESSION['is_global_admin'] ?? false)) {
        continue;
    }
    if (!in_array($key, $ignore_columns))
    {
    // NEW: Tenant-Admins sollen is_global_admin nicht sehen
     if ($key === 'is_global_admin' && !($_SESSION['is_global_admin'] ?? false)) {
       continue;
     }     
     $col_value = $row[$key];
      ...
      switch($key)
      {
        ...
        case 'level':
    // Level nur anzeigen – keine Manipulation hier!
    $values[] = htmlspecialchars((string)$row[$key]);
    break;
    ...
    // User darf sich nicht selbst hochstufen
...
// Multi-Tenant: Global-Admins sehen alles, Tenant-Admins nur ihren Tenant
$is_global_admin = (int)($_SESSION['is_global_admin'] ?? 0);
$is_tenant_admin = (int)($_SESSION['is_tenant_admin'] ?? 0);
$tenant_id       = $_SESSION['tenant_id']       ?? null;

if (!$is_global_admin && $is_tenant_admin && $tenant_id !== null)
{
  // array_values(): neue Indizes 0..N für DataTables
  $users = array_values(array_filter($users, function($u) use ($tenant_id) {
    $u_tenant = $u['tenant_id'] ?? null;
    return ((int)$u_tenant === (int)$tenant_id);
  }));
}
...
if (count($users) > 0)
{
  ...
  $current_user_id = (int)($mrbs_user->id ?? 0);

  if (isset($id) && ($id === $current_user_id)) {
      // Self-edit immer erlauben
  }
  else {
      // Normale Rechteprüfung
      checkAuthorised(this_page());
  }
}
else
{
  // We've just created the table...
...
if (isset($action) && ( ($action == "edit") or ($action == "add") ))
{
  ...
  if (!$initial_user_creation && !can_edit_user($data))
  {
    showAccessDenied();
    exit();
  }
  ...
  foreach ($fields as $field)
  {
   ...
   $is_self_edit = isset($id) && ($id == $current_user_id);

   $disabled = (
    !$initial_user_creation &&
    !is_user_admin() &&
    !$is_self_edit &&           // Self-edit NICHT sperren
    in_array($key, $auth['db']['protected_fields'])
   ); 
    // 🔒 NEU: Tenant-Admins und normale User sehen is_global_admin NICHT
    if ($key === 'is_global_admin' && !($_SESSION['is_global_admin'] ?? false)) {
        continue;
    }
    ...
    $editing_last_admin ||
    $disabled ||
    !($_SESSION['is_global_admin'] ?? false);  // NEW: nur Global Admins dürfen Level ändern
...
// --- Sicherheitscheck: Tenant-Admins dürfen NIE is_global_admin setzen ---
if (!($_SESSION['is_global_admin'] ?? false)) {
    unset($_POST['is_global_admin']);
    unset($_REQUEST['is_global_admin']);
    unset($GLOBALS['HTTP_VARS_CACHE']['f_is_global_admin']);
}
  ...
  // Erlaubt: Admins oder Self-Edit
  if (!$initial_user_creation && !is_user_admin() && $id !== $current_user_id) {
...
{
// FINALER SCHUTZ: Tenant-Admins dürfen nie global_admin setzen
if (!($_SESSION['is_global_admin'] ?? false)) {
    // Entfernt Input vor get_form_var()
    $_POST['f_is_global_admin'] = 0;
    $_REQUEST['f_is_global_admin'] = 0;

    // Zusätzlich: MRBS interner get_form_var Cache löschen
    unset($GLOBALS['HTTP_VARS_CACHE']['f_is_global_admin']);
}
...
if (!$is_ajax)
{
  ...
  if (is_user_admin() || !empty($_SESSION['is_global_admin']) || !empty($_SESSION['is_tenant_admin']))
  {
    $form = new Form(Form::METHOD_POST);
...
if ($initial_user_creation != 1)   // don't print the user table if there are no users
{
  ...
  // NEW: Tenant-Admins sehen is_global_admin nicht
  if (!($_SESSION['is_global_admin'] ?? false)) {
    $ignore_columns[] = 'is_global_admin';
  }    
  ...
  // Wenn wir Ajax-fähig sind und dies ist eine Ajax-Request (DataTables),
  // dann liefern wir hier nur JSON-Daten zurück.
