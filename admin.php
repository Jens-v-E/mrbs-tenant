...
// -----------------------------------------------------------
// Sicherheitscheck für Custom Fields Manager
// Nur globale Admins dürfen den Custom-Fields-Bereich betreten
// -----------------------------------------------------------
if (isset($_GET['custom_fields_manager']) && $_GET['custom_fields_manager'] == 1)
{
    if (empty($_SESSION['is_global_admin'])) {
        // Benutzer hat keine Rechte -> zurück zur Startseite
...
function generate_area_change_form(array $enabled_areas, array $disabled_areas) : void
{
  ...
  // The area select
// -----------------------------
// AREA-FILTER NACH TENANT
// -----------------------------

// Bereiche nach Tenant- und Userrechten filtern
$visible_enabled_areas  = [];
$visible_disabled_areas = [];

foreach ($enabled_areas as $a_id => $a_name) {
    if (is_area_visible((int)$a_id)) {
        $visible_enabled_areas[$a_id] = $a_name;
    }
}

foreach ($disabled_areas as $a_id => $a_name) {
    if (is_area_visible((int)$a_id)) {
        $visible_disabled_areas[$a_id] = $a_name;
    }
}

// Bereichsliste je nach Rolle aufbauen
if (is_admin()) {
    // Global Admin sieht alle (aber gefiltert wg Tenant Admin)
    $options = array(
        get_vocab("enabled")  => $visible_enabled_areas,
        get_vocab("disabled") => $visible_disabled_areas
    );
}
else {
    // Tenant-Admin / normaler User / Gast → nur sichtbare aktive Bereiche
    $options = $visible_enabled_areas;
}
...
// Prüfen, ob Custom Fields Manager aufgerufen werden soll
if (get_form_var('custom_fields_manager', 'int') === 1) {
    require_once __DIR__ . '/local/custom_fields_manager.php';
    exit; // Alles weitere in admin.php wird übersprungen
}
